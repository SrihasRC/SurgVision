"""
================================================================================
YOLOV11 SEGMENTATION - DISTANCE MONITORING (No Smoothing)
================================================================================
Surgical instrument distance measurement from anatomical structures.
Raw per-frame detection, no temporal smoothing.
...
class DistanceMonitoringProcessor:
    ...
    def calculate_distances(self, masks, classes, confidences):
        ...
        is_debug = self.debug_frame_count < 5
        ...
        if not detections['instrument'] or not detections['organs']:
            if is_debug:
                if not detections['instrument']:
                    print(f"   [DEBUG] ❌ No instrument detected!")
                if not detections['organs']:
                    print(f"   [DEBUG] ❌ No organs detected!")
            return []
    ...
    def process_frame(self, frame):
        ...
        output = self.draw_segmentation_boundaries(frame, masks, classes, confs)
        distances = self.calculate_distances(masks, classes, confs)
        ...
        try:
            with tqdm(total=total_frames, desc="   Progress", ncols=100) as pbar:
                while True:
                    ...
                    cv2.putText(
                        final, "DISTANCE MONITORING (RAW)", (w + w // 2 - 180, 25),
                    ...
                    if show_preview:
                        ...
                        if scale < 1.0:
                            ...
                            display = final
                        cv2.imshow('Surgical Distance Monitoring (No Smoothing)', display)
...
def main():
    parser = argparse.ArgumentParser(
        description="Surgical instrument distance monitoring (NO smoothing)"
    )
    parser.add_argument('input_video', help="Path to input video")
    parser.add_argument('-o', '--output', default=None, help="Output video path (saved in current dir by default)")
    ...
    parser.add_argument('--show-preview', action='store_true', help="Show live CV2 preview")
    ...
    if not args.output:
        ...
        args.output = f"{Path(args.input_video).stem}_distances_nosmooth.mp4"
